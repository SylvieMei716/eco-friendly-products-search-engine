...
from ranker import Ranker, CrossEncoderScorer, TF_IDF, PivotedNormalization, BM25
...
class L2RRanker:
    ...
    def query(self, query: str, pseudofeedback_num_docs=0, pseudofeedback_alpha=0.8,
        ...
        relevant_docs = self.scorer.query(query)
...
class L2RFeatureExtractor:
    def __init__(self, document_index: InvertedIndex, title_index: InvertedIndex,
                 ...
                 ce_scorer: CrossEncoderScorer, keywords) -> None:
        """
    ...
    """def get_keyword_similarity(self, docid: int, keyword_embed: np.ndarray, prod_embeddings: np.ndarray, row_to_doc: list):
        keyword_similarity = Similarity(self.document_index, docid, keyword_embed,  
                                        prod_embeddings, row_to_doc, False)
        return keyword_similarity.Cosine_similarity()"""
    
    def get_tf_idf_keyword(self, index: InvertedIndex, docid: int,
                word_counts: dict[str, int], keyword_dict: dict[str, int]) -> float:
